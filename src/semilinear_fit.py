...
def fit_bayesian(x, y, x_out=None, M=9, frequencies=[], **kwds) -> dict:
    ...
    prediction, std = model.predict(feature_matrix(
        x_out, M, frequencies), return_std=True)
...
if __name__ == '__main__':
    ...
    def signal(x, noise_amount=0.25):
        harmonics = 0.05 * np.sin(x * 6.4) + 0.3 * \
            np.sin(x * 2.234 + 1) + 0.2 * np.sin(x * 0.8)
    ...
    for i, ax in enumerate(axes):
        ...
        if i == 0:
            y_prediction, std = fit_bayesian(
                x_random, y_observed, x_linear, M=7, frequencies=[0.8, 6.4])
        else:
            # note that combining multiple linear kernels (e.g. dotproduct)
            # allows for nonlinear models
            ...
            y_prediction, std = model.predict(
                x_linear.reshape(-1, 1), return_std=True)

        rel_rmse = np.sqrt(mse(y_true, y_prediction)) / y_true.sum()
        rel_mae = np.abs(y_true - y_prediction).sum() / y_true.sum()
        print(
            f'{titles[i]:<20} rel mae: {rel_mae:.4f} \trel rmse: {rel_rmse:6f}')
    ...
    n_samples = 10
    plt.plot(x, model.sample_y(x.reshape(-1, 1), n_samples),
             alpha=0.2, label='sampled')
    plt.tight_layout()
